#define kKiB (uword)(1024)
#define kMiB (uword)(kKiB * kKiB)
#define kGiB (uword)(kKiB * kKiB * kKiB)
#define kPageSize (uword)(4 * kKiB)
#define kSmallIntTagBits (uword)(1)
#define kPrimaryTagBits (uword)(3)
#define kImmediateTagBits (uword)(5)
#define kSmallIntTagMask (uword)((1 << kSmallIntTagBits) - 1)
#define kPrimaryTagMask (uword)((1 << kPrimaryTagBits) - 1)
#define kImmediateTagMask (uword)((1 << kImmediateTagBits) - 1)
#define kWordSize (uword)(sizeof(word))
#define kMaxSmallStringLength (uword)(kWordSize - 1)
#define kBitsPerByte (uword)(8)
#define kSmallIntTag (uword)(0)
#define kHeapObjectTag (uword)(1)
#define kEmptyListTag (uword)(5)
#define kHoleTag (uword)(7)
#define kSmallStringTag (uword)(13)
#define kBitsPerPointer (uword)(kBitsPerByte * kWordSize)
#define kSmallIntBits (word)(kBitsPerPointer - kSmallIntTagBits)
#define kSmallIntMinValue (word)(-(((word)1) << (kSmallIntBits - 1)))
#define kSmallIntMaxValue (word)((((word)1) << (kSmallIntBits - 1)) - 1)
#include <assert.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
__attribute__((section("const_heap"),
               used)) struct heap_string private_unused_const_heap = {
...
struct object* page_0(struct object* this, struct object* body);
struct object* status_6(struct object* this, struct object* code);
struct object* fn_7(struct object* this, struct object* body);
struct object* Record_builder_body_code(struct object* body,
                                        struct object* code);
struct object* fn_18(struct object* this, struct object* tmp_17);
#define ptrto(obj) ((struct object*)((uword) & (obj) + 1))
#define CONST_HEAP __attribute__((section("const_heap")))
CONST_HEAP struct heap_string const_heap_string_1 = {
    .HEAD.tag = TAG_STRING, .size = 27, .data = "<!doctype html><html><body>"};
CONST_HEAP struct heap_string const_heap_string_3 = {
    .HEAD.tag = TAG_STRING, .size = 14, .data = "</body></html>"};
CONST_HEAP struct closure const_closure_5 = {
    .HEAD.tag = TAG_CLOSURE, .fn = page_0, .size = 0};
CONST_HEAP struct closure const_closure_12 = {
    .HEAD.tag = TAG_CLOSURE, .fn = status_6, .size = 0};
CONST_HEAP struct heap_string const_heap_string_14 = {
    .HEAD.tag = TAG_STRING, .size = 9, .data = "not found"};
CONST_HEAP struct heap_string const_heap_string_23 = {
    .HEAD.tag = TAG_STRING, .size = 19, .data = "you're on the index"};
CONST_HEAP struct heap_string const_heap_string_27 = {
    .HEAD.tag = TAG_STRING, .size = 24, .data = "you're on the about page"};
struct object* scrap_main() {
  HANDLES();
  OBJECT_HANDLE(
      tmp_13, closure_call(ptrto(const_closure_12),
                           (struct object*)(((uword)404 << kSmallIntTagBits))));
  OBJECT_HANDLE(tmp_15, closure_call(ptrto(const_closure_5),
                                     ptrto(const_heap_string_14)));
  OBJECT_HANDLE(tmp_16, closure_call(tmp_13, tmp_15));
  OBJECT_HANDLE(tmp_30, mkclosure(heap, fn_18, 3));
  closure_set(tmp_30, /*notfound=*/0, tmp_16);
  closure_set(tmp_30, /*page=*/1, ptrto(const_closure_5));
  closure_set(tmp_30, /*status=*/2, ptrto(const_closure_12));
  return tmp_30;
}
struct object* page_0(struct object* this, struct object* body) {
  HANDLES();
  GC_PROTECT(this);
  GC_PROTECT(body);
  if (!(is_string(ptrto(const_heap_string_1)))) {
    fprintf(stderr, "assertion is_string(ptrto(const_heap_string_1)) failed\n");
    abort();
  }
  if (!(is_string(body))) {
    fprintf(stderr, "assertion is_string(body) failed\n");
    abort();
  }
  OBJECT_HANDLE(tmp_2, string_concat(ptrto(const_heap_string_1), body));
  if (!(is_string(tmp_2))) {
    fprintf(stderr, "assertion is_string(tmp_2) failed\n");
    abort();
  }
  if (!(is_string(ptrto(const_heap_string_3)))) {
    fprintf(stderr, "assertion is_string(ptrto(const_heap_string_3)) failed\n");
    abort();
  }
  OBJECT_HANDLE(tmp_4, string_concat(tmp_2, ptrto(const_heap_string_3)));
  return tmp_4;
}
struct object* status_6(struct object* this, struct object* code) {
  HANDLES();
  GC_PROTECT(this);
  GC_PROTECT(code);
  OBJECT_HANDLE(tmp_11, mkclosure(heap, fn_7, 1));
  closure_set(tmp_11, /*code=*/0, code);
  return tmp_11;
}
struct object* fn_7(struct object* this, struct object* body) {
  HANDLES();
  GC_PROTECT(this);
  GC_PROTECT(body);
  OBJECT_HANDLE(tmp_8, closure_get(this, /*code=*/0));
  OBJECT_HANDLE(tmp_10, Record_builder_body_code(body, tmp_8));
  return tmp_10;
}
struct object* Record_builder_body_code(struct object* body,
                                        struct object* code) {
  ...
  GC_PROTECT(body);
  GC_PROTECT(code);
  OBJECT_HANDLE(tmp_9, mkrecord(heap, 2));
  record_set(tmp_9, /*index=*/0,
             (struct record_field){.key = Record_body, .value = body});
  record_set(tmp_9, /*index=*/1,
             (struct record_field){.key = Record_code, .value = code});
  return tmp_9;
}
struct object* fn_18(struct object* this, struct object* tmp_17) {
  HANDLES();
  GC_PROTECT(this);
  GC_PROTECT(tmp_17);
  OBJECT_HANDLE(tmp_19, closure_get(this, /*notfound=*/0));
  OBJECT_HANDLE(tmp_20, closure_get(this, /*page=*/1));
  OBJECT_HANDLE(tmp_21, closure_get(this, /*status=*/2));
  if (tmp_17 != mksmallstring("/", 1)) {
    goto case_1;
  }
  OBJECT_HANDLE(
      tmp_22,
      closure_call(tmp_21, (struct object*)(((uword)200 << kSmallIntTagBits))));
  OBJECT_HANDLE(tmp_24, closure_call(tmp_20, ptrto(const_heap_string_23)));
  OBJECT_HANDLE(tmp_25, closure_call(tmp_22, tmp_24));
  return tmp_25;
case_1:;
  if (tmp_17 != mksmallstring("/about", 6)) {
    goto case_2;
  }
  OBJECT_HANDLE(
      tmp_26,
      closure_call(tmp_21, (struct object*)(((uword)200 << kSmallIntTagBits))));
  OBJECT_HANDLE(tmp_28, closure_call(tmp_20, ptrto(const_heap_string_27)));
  OBJECT_HANDLE(tmp_29, closure_call(tmp_26, tmp_28));
  return tmp_29;
case_2:;
  return tmp_19;
no_match:;
